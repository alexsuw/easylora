...
- Resolved CI regressions in lint/typecheck/format checks introduced during autopilot integration.
- Added sharded multi-GPU training support for `device_map: "auto"` models by enabling
  model-parallel flags before HF Trainer setup, avoiding DataParallel placement conflicts.
- Added regression coverage for sharded vs non-sharded trainer behavior.
...
- Made `bitsandbytes` a core dependency (installed automatically with easylora).
